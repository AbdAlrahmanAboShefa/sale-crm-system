...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="dark-stats-card" style="--card-accent: #3b82f6; --card-accent-secondary: #2563eb;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">{{ __('messages.dashboard.my_deals') ?? 'My Deals' }}</p>
                    <p class="text-3xl font-bold text-slate-100 mt-2 tracking-tight">{{ $stats['totalDeals'] }}</p>
                    <p class="text-xs text-slate-500 mt-2">Assigned to you</p>
                </div>
        ...
        <div class="dark-stats-card" style="--card-accent: #8b5cf6; --card-accent-secondary: #7c3aed;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">{{ __('messages.dashboard.my_pipeline_value') ?? 'My Pipeline Value' }}</p>
                    <p class="text-3xl font-bold text-slate-100 mt-2 tracking-tight">${{ number_format($stats['totalValue'], 0) }}</p>
                    <p class="text-xs text-slate-500 mt-2">Your pipeline</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 dark-card p-6">
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-slate-100">{{ __('messages.dashboard.my_pipeline') ?? 'My Pipeline' }}</h3>
                <p class="text-xs text-slate-500 mt-1">Your deal progression</p>
            </div>
            <div class="space-y-5">
        ...
        <div class="dark-chart-container">
            <div class="mb-6">
                ...
                <p class="text-xs text-slate-500 mt-1">Your distribution</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="dark-card p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h3 class="text-lg font-semibold text-slate-100">{{ __('messages.dashboard.my_recent_activities') ?? 'My Recent Activities' }}</h3>
                    <p class="text-xs text-slate-500 mt-1">Latest interactions</p>
                </div>
                <a href="{{ route('agent.activities.index') }}" class="text-sm text-cyan-400 hover:text-cyan-300 transition-colors flex items-center gap-1">
        ...
        <div class="dark-card p-6">
            <div class="mb-6">
                ...
                <p class="text-xs text-slate-500 mt-1">Your monthly closings</p>
            </div>
