        ...
        <form action="{{ route($routePrefix . '.contacts.update', $contact) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $contact->name) }}" required class="dark-input w-full">
                </div>
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $contact->email) }}" required class="dark-input w-full">
                </div>
                <div>
                    <label for="phone" class="block text-sm font-medium text-slate-300 mb-1">{{ __('messages.contacts.phone') }}</label>
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $contact->phone) }}" class="dark-input w-full">
                </div>
                <div>
                    <label for="company" class="block text-sm font-medium text-slate-300 mb-1">{{ __('messages.contacts.company') }}</label>
                    <input type="text" name="company" id="company" value="{{ old('company', $contact->company) }}" class="dark-input w-full">
                </div>
                <div>
                    <label for="website" class="block text-sm font-medium text-slate-300 mb-1">{{ __('messages.common.website') }}</label>
                    <input type="url" name="website" id="website" value="{{ old('website', $contact->website) }}" placeholder="https://" class="dark-input w-full">
                </div>
                <div>
            ...
            <div class="mt-6">
                ...
                <input type="text" name="tags_input" id="tags_input" placeholder="{{ __('messages.common.tags_placeholder') }}" value="{{ old('tags_input', $contact->tags ? implode(', ', $contact->tags) : '') }}" class="dark-input w-full">
            ...
            <div class="flex justify-end gap-3 mt-8">
                ...
                <button type="submit" class="dark-btn dark-btn-primary">{{ __('messages.common.edit') }} {{ __('messages.contacts.title') }}</button>
            </div>
        </form>
    </div>
@endsection
