...
<head>
    ...
    <title>Activities - Sales CRM</title>
...
<body class="bg-gray-50">
    <div class="min-h-screen flex">
        ...
        <div class="flex-1 flex flex-col">
            <header class="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                <div><h2 class="text-xl font-semibold text-gray-800">Activities</h2><p class="text-sm text-gray-500">{{ $activities->total() }} total activities</p></div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                    <div class="p-6 border-b border-gray-100">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form method="GET" class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <select name="is_done" onchange="this.form.submit()" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                                        ...
                                        <option value="0" {{ request('is_done') === '0' ? 'selected' : '' }}>Pending</option>
                            ...
                            <a href="{{ route('agent.activities.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 rounded-lg text-sm font-medium text-white hover:bg-blue-700 shadow-sm transition-colors">
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="{{ $activity->isOverdue() ? 'border-l-4 border-red-500 bg-red-50' : '' }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-sm text-gray-600 max-w-xs truncate">{{ $activity->note ?? 'No notes' }}</p>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        @if($activity->is_done)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">Done</span>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            @if(!$activity->is_done)
                                            <button onclick="markDone({{ $activity->id }})" class="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Mark as Done">
                                                <i class="fas fa-check"></i>
                                            </button>
                                            @endif
                                            <a href="{{ route('agent.activities.edit', $activity) }}" class="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                                <i class="fas fa-pencil"></i>
                                            </a>
                                            <form action="{{ route('agent.activities.destroy', $activity) }}" method="POST" x-data="{ showModal: false }" class="inline">
                                                @csrf @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 mb-1">No activities found</h3>
                                            <p class="text-gray-500 mb-4">Log your first activity to get started.</p>
                                            <a href="{{ route('agent.activities.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 rounded-lg text-sm font-medium text-white hover:bg-blue-700">
                                                <i class="fas fa-plus mr-2"></i>Log Activity
                                            </a>
                                        </div>
                                    </td>
    ...
    function markDone(id) {
        fetch(`{{ url('/') }}/agent/activities/${id}/done`, {
            method: 'PATCH',
