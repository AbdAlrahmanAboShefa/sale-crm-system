...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-100 tracking-tight">{{ __('messages.nav.dashboard') }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="dark-stats-card" style="--card-accent: #3b82f6; --card-accent-secondary: #2563eb;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">{{ __('messages.deals.all_deals') }}</p>
                    <p class="text-3xl font-bold text-slate-100 mt-2 tracking-tight">{{ $stats['totalDeals'] }}</p>
                    <p class="text-xs text-slate-500 mt-2">Total deals created</p>
                </div>
        ...
        <div class="dark-stats-card" style="--card-accent: #10b981; --card-accent-secondary: #059669;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">{{ __('messages.common.active') }} {{ __('messages.deals.title') }}</p>
                    <p class="text-3xl font-bold text-slate-100 mt-2 tracking-tight">{{ $stats['activeDeals'] }}</p>
                    <p class="text-xs text-slate-500 mt-2">Currently in pipeline</p>
                </div>
        ...
        <div class="dark-stats-card" style="--card-accent: #8b5cf6; --card-accent-secondary: #7c3aed;">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-2">Total pipeline value</p>
                </div>
        ...
        <div class="dark-stats-card" style="--card-accent: #f59e0b; --card-accent-secondary: #d97706;">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-400">{{ __('messages.deals.stages.won') }} {{ __('messages.deals.title') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 dark-card p-6">
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-slate-100">{{ __('messages.deals.pipeline_value') }}</h3>
                <p class="text-xs text-slate-500 mt-1">Deal stage progression</p>
            </div>
            <div class="space-y-5">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm font-medium text-slate-300">{{ __("messages.deals.stages." . strtolower($stage)) }}</span>
        ...
        <div class="dark-chart-container">
            ...
            <div class="grid grid-cols-2 gap-3 mt-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-slate-400">{{ __("messages.deals.stages." . strtolower($stage)) }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="dark-card p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.activities.index') }}" class="text-sm text-cyan-400 hover:text-cyan-300 transition-colors flex items-center gap-1">
            ...
            <div class="space-y-4">
                ...
                <div class="dark-activity-item group">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-500 truncate mt-0.5">{{ $activity->note ?: __('messages.activities.note') }}</p>
                        <div class="flex items-center gap-3 mt-2">
                            ...
                            @elseif($activity->isOverdue())
                                <span class="inline-flex items-center gap-1 text-xs text-rose-400">
                                    <i class="fas fa-exclamation-circle"></i> {{ __('messages.activities.pending') }}
                                </span>
                            @endif
                        </div>
                    </div>
        ...
        <div class="dark-card p-6">
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-slate-100">{{ __('messages.dashboard.my_performance') }}</h3>
                <p class="text-xs text-slate-500 mt-1">Monthly deal closings</p>
            </div>
